...
public class AudioRecorder {
        ...
        try {
            if (filesName.size() > 0) {
                ...
                for (String fileName : filesName) {
                    filePaths.add(new FileUtils().getPcmFileAbsolutePath(fileName));
                }
                //清除
                filesName.clear();
                //将多个pcm文件转化为wav文件
    ...
    private void writeDataTOFile(String currentFileName) {
        ...
        int readsize = 0;
        try {
            File file = new File(new FileUtils().getPcmFileAbsolutePath(currentFileName));
            if (file.exists()) {
                file.delete();
            }
            fos = new FileOutputStream(file);// 建立一个可存取字节的文件
            System.out.println("可存取字节的文件"+file);
        } catch (IllegalStateException e) {
